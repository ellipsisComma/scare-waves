...
function getRandomShowID() {
	...
	const pool = [...document.querySelectorAll(`#series-list .show-list > li`)].filter(show => {
...
function addRandomShow() {
	...
	} else console.warn(`Can't add random new show: all shows already in queue.`);
}
...
function initialise() {
	...
	document.getElementById(`series-list`).addEventListener(`click`, () => {
		if (event.target.tagName === `BUTTON` && `action` in event.target.dataset) {
			...
			if (button.dataset.action === `add-show`) {
				addShow(button.closest(`.show-list > li`).dataset.showId);
